...
def exact_gate(state: pd.DataFrame, perf: pd.DataFrame) -> tuple[bool, str, str]:
    ...
    if "raw_target_feature_source" not in state.columns or not state["raw_target_feature_source"].dropna().astype(str).eq("raw_target_return_exact").any():
...
def build_official_from_candidate(state: pd.DataFrame, date: str, overwrite_same_day: bool, data_status: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for t in sorted(set(prev_weights) | set(weights)):
        ...
        rows.append({"date":date,"signal_date":date,"economic_application_date":next_business_day(date),"ticker":t,"action":action,"old_weight":old_w,"new_weight":new_w,"weight_change":ch,"old_position_value":old_w*portfolio_value,"new_position_value":new_w*portfolio_value,"estimated_trade_value":abs(ch)*portfolio_value,"execution_price":prices.get(t,np.nan),"reason":reason,"raw_target_rank":m.get("raw_target_rank",np.nan),"raw_target_return_exact":m.get("raw_target_return_exact",np.nan),"holding_quality_classification":m.get("holding_quality_classification",np.nan),"holding_risk_notes":m.get("holding_risk_notes","")})
    cash = float(cash_rows.iloc[-1].get("paper_position_weight", 1.0-sum(weights.values()))) if not cash_rows.empty else 1.0-sum(weights.values())
    old_cash = 1.0-sum(prev_weights.values()) if not first_official else 1.0
    rows.append({"date":date,"signal_date":date,"economic_application_date":next_business_day(date),"ticker":"CASH","action":"CASH_CHANGE" if abs(cash-old_cash)>1e-8 else "HOLD","old_weight":old_cash,"new_weight":cash,"weight_change":cash-old_cash,"old_position_value":old_cash*portfolio_value,"new_position_value":cash*portfolio_value,"estimated_trade_value":abs(cash-old_cash)*portfolio_value,"execution_price":1.0,"reason":"official_cash_rebalance"})
    ...
    if not trades.empty:
        trades["previous_weight"]=trades["old_weight"]; trades["trade_weight_change"]=trades["weight_change"]; trades["model_mode"]=MODEL; trades["growth_paper_variant"]=VARIANT
    ...
    for _, r in current.iterrows():
        t=str(r["ticker"]).upper(); w=float(r.get("paper_position_weight",0.0)); px=float(r.get("current_price",np.nan)); entry=px if first_official or action_map.get(t)=="BUY" else px
        state_rows.append({**r.to_dict(),"date":date,"signal_date":date,"economic_application_date":next_business_day(date),"paper_position_weight":w,"paper_position_value":portfolio_value*w,"entry_price":entry,"current_price":px,"action":action_map.get(t,"HOLD"),"model_mode":MODEL,"growth_model_version":MODEL_VERSION,"model_version":MODEL_VERSION,"official_forward_namespace":True,"data_mode":"official_forward_exact","unrealized_return": 0.0 if first_official or action_map.get(t)=="BUY" else r.get("unrealized_return", np.nan),"realized_return": daily_ret,"real_orders":False})
    state_rows.append({"date":date,"signal_date":date,"economic_application_date":next_business_day(date),"ticker":"CASH","paper_position_weight":cash,"paper_position_value":portfolio_value*cash,"entry_price":1.0,"current_price":1.0,"action":"HOLD","model_mode":MODEL,"growth_model_version":MODEL_VERSION,"model_version":MODEL_VERSION,"official_forward_namespace":True,"data_mode":"official_forward_exact","unrealized_return": 0.0 if first_official or action_map.get(t)=="BUY" else r.get("unrealized_return", np.nan),"realized_return": daily_ret,"real_orders":False})
    if not trades.empty:
    ...
    gross_series=pd.concat([pd.to_numeric(perf_existing.get("gross_daily_return", pd.Series(dtype=float)), errors="coerce"), pd.Series([daily_ret])], ignore_index=True).fillna(0.0)
    equity=(1+gross_series).cumprod(); dd=equity/equity.cummax()-1
    turnover=float(actions[actions["ticker"].ne("CASH")]["weight_change"].abs().sum())
    perf=pd.DataFrame([{"date":date,"signal_date":date,"economic_application_date":next_business_day(date),"model_mode":MODEL,"growth_paper_variant":VARIANT,"growth_model_version":MODEL_VERSION,"model_version":MODEL_VERSION,"portfolio_value":portfolio_value,"daily_return":daily_ret,"gross_daily_return":daily_ret,"estimated_net_daily_return":daily_ret-(estimated_execution_cost/portfolio_value if portfolio_value else 0.0),"cumulative_return":float(equity.iloc[-1]-1),"gross_cumulative_return":float(equity.iloc[-1]-1),"estimated_net_cumulative_return":float(equity.iloc[-1]-1),"volatility":float(gross_series.std(ddof=0)*np.sqrt(252)) if len(gross_series)>1 else 0.0,"Sharpe":0.0,"max_drawdown":float(dd.min()),"cash_weight":cash,"exposure":float(sum(weights.values())),"turnover":turnover,"estimated_execution_cost":estimated_execution_cost,"estimated_cost_reporting_only":True,"integrity_status":"PASS","data_status":data_status,"official_forward_namespace":True,"data_mode":"official_forward_exact","real_orders":False}])
...
def run_official_lifecycle(overwrite_same_day: bool = False) -> dict[str, object]:
    ...
    official_state, official_actions, official_trades, official_perf = build_official_from_candidate(state, date, overwrite_same_day, reason)
    written = {
    ...
    official_rebalance_report = pd.DataFrame([{
        ...
        "current_non_cash_holdings": ",".join(official_state[official_state["ticker"].astype(str).str.upper().ne("CASH")]["ticker"].astype(str).tolist()),
    ...
    monitor = pd.DataFrame([{
        "date": date, "model": MODEL, "model_version": MODEL_VERSION,
        ...
        "cash": perf_row.get("cash_weight", np.nan), "exposure": perf_row.get("exposure", np.nan),
        "turnover": perf_row.get("turnover", np.nan), "governance_status": "official_forward_warmup",
        "promotion_status": "real_capital_blocked", "data_status": reason, "integrity_status": "PASS",
        "risk_flags": "official_forward_warmup", "real_orders": False,
    }])
    ...
    integrity = pd.DataFrame([{
        "date": date, "model_version": MODEL_VERSION, "raw_target_exact": True, "fresh_canonical_ohlcv": True,
        "exact_five_session_scheduler": True, "rebalance_due": bool(schedule.get("rebalance_due", False)),
        "next_rebalance": schedule.get("next_rebalance_date", ""), "proxy_used": False,
        "stale_override_used": False, "integrity_status": "PASS", "data_status": reason,
    }])
    integrity.to_csv(INTEGRITY_STATUS, index=False)
    status = pd.DataFrame([{
        "date": date, "run_time": datetime.now().isoformat(timespec="seconds"), "model_version": MODEL_VERSION,
        "official_updated": True, "status": "ok", "rebalance_due": bool(schedule.get("rebalance_due", False)),
        "monitoring_only": bool(not schedule.get("rebalance_due", False)), "next_rebalance": schedule.get("next_rebalance_date", ""),
        "state_rows_added": written["state"][0], "trades_rows_added": written["trades"][0],
        "actions_rows_added": written["actions"][0], "performance_rows_added": written["performance"][0],
        "monitor_rows_added": len(monitor), "data_status": reason, "integrity_status": "PASS", "real_orders": False,
    }])
    append_or_update(DAILY_STATUS, status, date, overwrite_same_day)
    version = pd.DataFrame([{
        "date": date, "model_version": MODEL_VERSION, "model": MODEL, "variant": VARIANT, "frozen_parameters": True,
        "target_volatility": 0.22, "minimum_exposure": 0.40, "exposure_cap": 0.60,
        "volatility_lookback_days": 60, "dual_trend_caps": "60/40/25", "max_positions": 4,
        "allocation_method": "equal_weight_within_final_exposure", "real_capital_status": "real_capital_blocked",
    }])
    ...
    return {"status": "ok", "date": date, "holdings": ",".join(holdings["ticker"].astype(str).tolist()), "data_status": reason, "integrity_status": "PASS"}
