...
def _system_uses_yfinance() -> tuple[bool, str]:
    ...
    return uses, "financial_data_system.py imports yfinance as yf and calls yf.download" if uses else "No direct yfinance download call detected"
...
def yahoo_ohlcv_coverage(universe: str, batch_size: int, sleep_seconds: float, retries: int, reset: bool, max_tickers: int | None) -> pd.DataFrame:
...
def feature_reconstructability() -> pd.DataFrame:
    rows = [
        ["raw_target_return_exact", "C_requires_historical_forecast_snapshots_for_exact", False, "approximate_if_full_target_engine_rerun", True, True, "Exact value is target_price/current_price generated by model; OHLCV alone cannot recover stored pre-2022 targets."],
        ["raw_target_rank", "B_reconstructable_from_current_model_logic_after_raw_target", False, True, True, True, "Deterministic only after raw_target_return exists."],
        ["soft_exit_rule", "B_reconstructable_from_current_model_logic_after_signals", False, True, False, True, "Needs prior selected positions and current raw target sign."],
        ["volatility_target_22pct", "A_reconstructable_from_strategy_return_path", "after_candidate_returns_exist", True, False, False, "Causal once candidate returns are computed."],
        ["exposure_cap_60", "A_reconstructable_from_rule", True, True, False, False, "Fixed cap; no market data dependency."],
        ["final_allocation", "B_reconstructable_from_current_model_logic_after_selection", False, True, "for_exact_replay", True, "Needs selected tickers, soft exit state, volatility target and cap."],
        ["current_optimizer_constraints", "not_active_in_growth_v2_replay", "not_required", "not_required", False, False, "Growth v2 paper construction is overlay-based, not optimizer-driven."],
        ["benchmark_SPY_QQQ_comparison", "A_reconstructable_from_ohlcv", True, True, False, False, "Yahoo OHLCV can provide benchmark returns if download succeeds."],
    ]
    out = pd.DataFrame(rows, columns=["component", "classification", "reconstructable_from_ohlcv", "reconstructable_from_current_model_logic", "requires_historical_forecast_snapshots", "not_reconstructable_before_2022_exact", "notes"])
...
def reconstructed_backtest_plan(coverage: pd.DataFrame, total_universe: int, universe: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    plan = pd.DataFrame([
        {"plan_step": 1, "name": "batched_price_data_download", "description": "Download adjusted OHLCV from Yahoo in batches with retry and partial progress.", "exact_or_approx": "input_preparation"},
        {"plan_step": 2, "name": "causal_model_rerun", "description": "For each date t, truncate OHLCV to t and rerun current target engine to approximate raw target.", "exact_or_approx": "approximation_not_snapshot_replay"},
        {"plan_step": 3, "name": "growth_overlay", "description": "Apply raw target rank, soft_exit_rule, volatility_target_22pct and exposure_cap_60 causally.", "exact_or_approx": "causal_reconstruction"},
        {"plan_step": 4, "name": "label_and_benchmark", "description": "Compute future returns only after decisions and compare to SPY/QQQ.", "exact_or_approx": "walk_forward_evaluation"},
    ])
    ...
    if ok.empty:
        ...
        reason = "Yahoo OHLCV coverage exists for normal universe tickers before 2008; exact replay still impossible without pre-2022 snapshots."
    ...
    governance = pd.DataFrame([{
        ...
        "price_only_reconstruction_possible": classification in {"reconstructed_backtest_possible", "price_only_reconstruction_possible"},
...
def run(universe: str, batch_size: int, sleep_seconds: float, retries: int, reset: bool, max_tickers: int | None) -> None:
    ...
    print(f"total normal universe tickers: {total_universe}" if universe == "normal" else f"total CEDEAR universe tickers: {total_universe}")
    ...
    if not coverage.empty:
        ...
        cols = ["ticker", "download_status", "first_available_date", "last_available_date", "observations", "volume_available", "adjusted_close_available"]
        print(coverage[[c for c in cols if c in coverage.columns]].head(35).to_string(index=False))
    print("\n===== FEATURE RECONSTRUCTABILITY =====")
    print(reconstruct[["component", "classification", "requires_historical_forecast_snapshots", "not_reconstructable_before_2022_exact"]].to_string(index=False))
