...
def replay_dates() -> list[pd.Timestamp]:
    ...
    if fh.empty or "date" not in fh.columns:
        return []
            ...
            audit.append(
                {
                    "date": ds,
                    "status": "ok",
                    "rebalance_due": result.get("rebalance_due", ""),
                    "monitoring_only": result.get("monitoring_only", ""),
                    "portfolio_value": latest.iloc[-1].get("portfolio_value", "") if not latest.empty else "",
                    "daily_return": latest.iloc[-1].get("daily_return", "") if not latest.empty else "",
                }
            )
