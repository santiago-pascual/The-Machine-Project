...
def main() -> None:
    ...
    aligned = strategy[
        [
            "date",
            "growth_return",
            "growth_equity_recomputed",
            "growth_equity_reported",
            "growth_cumulative_return_recomputed",
            "exposure",
            "cash",
            "turnover",
        ]
    ...
    if not spy.empty:
        aligned = aligned.merge(
            spy[["date", "SPY_price", "SPY_return", "SPY_equity", "SPY_cumulative_return", "SPY_source_price_column"]],
            on="date",
            how="left",
        )
    if not qqq.empty:
        aligned = aligned.merge(
            qqq[["date", "QQQ_price", "QQQ_return", "QQQ_equity", "QQQ_cumulative_return", "QQQ_source_price_column"]],
            on="date",
            how="left",
        )
    ...
    rows.append(
        {
            "check": "same_initial_capital",
            "status": "PASS",
            "detail": f"Growth, SPY and QQQ normalized to {INITIAL_CAPITAL:,.0f} in audit recomputation.",
        }
    )
    norm_rows.append(
        {
            ...
            "first_date": strategy["date"].min().date(),
            "last_date": strategy["date"].max().date(),
            "final_equity": aligned["growth_equity_recomputed"].iloc[-1],
        }
    )
            ...
            norm_rows.append(
                {
                    ...
                    "first_date": strategy["date"].min().date(),
                    "last_date": strategy["date"].max().date(),
                    "final_equity": aligned[f"{ticker}_equity"].dropna().iloc[-1],
                }
            )
    ...
    for ticker in ["SPY", "QQQ"]:
        ...
        rows.append(
            {
                "check": f"{ticker}_adjusted_close_used",
                "status": status,
                "detail": source_col.iloc[-1] if not source_col.empty else "benchmark cache missing",
            }
        )
    ...
    rows.append(
        {
            "check": "growth_same_daily_frequency_as_benchmark",
            "status": "WARNING" if not exact_daily else "PASS",
            "detail": f"Strategy observations={len(strategy)}, median_gap_days={gaps.median() if not gaps.empty else 'n/a'}, max_gap_days={gaps.max() if not gaps.empty else 'n/a'}; not true daily if gaps > 1.",
        }
    )
    ...
    rows.append(
        {
            "check": "weekends_holidays_aligned",
            "status": "PASS" if weekend_dates == 0 else "WARNING",
            "detail": f"weekend_strategy_dates={weekend_dates}; benchmarks merge_asof to prior trading close.",
        }
    )
    ...
    rows.append(
        {
            "check": "volatility_targeting_exposure_bounds",
            "status": "PASS" if exp_ok else "WARNING",
            "detail": f"min_exposure={exposure.min()}, max_exposure={exposure.max()}, expected cap <=0.60",
        }
    )
    ...
    rows.append(
        {
            "check": "cash_included_correctly",
            "status": "PASS" if pd.notna(cash_identity) and cash_identity < 1e-9 else "WARNING",
            "detail": f"max_abs(exposure+cash-1)={cash_identity}",
        }
    )
    ...
    rows.append(
        {
            "check": "curves_normalized_identically",
            "status": "PASS" if norm_ok else "FAIL",
            "detail": "All audited series start from same initial capital.",
        }
    )
    ...
    rows.append(
        {
            "check": "growth_cumulative_return_calculation",
            "status": "PASS" if pd.notna(reported_diff) and reported_diff < 1e-6 else "WARNING",
            "detail": f"max_reported_vs_recomputed_equity_diff={reported_diff}",
        }
    )
        ...
        comp_rows.append(
            {
                ...
                "final_return_pct": (numeric(eq).dropna().iloc[-1] / INITIAL_CAPITAL - 1) * 100 if numeric(eq).notna().any() else np.nan,
                "max_compounding_diff": diff,
                "duplicated_compounding_suspected": bool(pd.notna(diff) and diff > 1e-6),
            }
        )
    rows.append(
        {
            "check": "no_duplicated_compounding",
            "status": "PASS" if all(not r["duplicated_compounding_suspected"] for r in comp_rows) else "FAIL",
            "detail": "See benchmark_compounding_check.csv",
        }
    )

    # 10 scaling mismatch
    scale_ok = aligned["growth_equity_recomputed"].iloc[-1] > INITIAL_CAPITAL and (
        "SPY_equity" not in aligned.columns or aligned["SPY_equity"].dropna().iloc[-1] > INITIAL_CAPITAL
    )
    rows.append(
        {
            "check": "no_scaling_mismatch",
            "status": "PASS" if scale_ok else "WARNING",
            "detail": "Equity values are dollar-normalized; chart values can be displayed as cumulative percent.",
        }
    )
