...
def build_kpis(data: dict[str, pd.DataFrame]) -> dict[str, Any]:
    ...
    return {
        ...
        "latest_research_update": max([r for r in [_date_range(_df(data, k)).split(" to ")[-1] if _date_range(_df(data, k)) else "" for k in RESEARCH_FILES] if r] or ["unavailable"]),
...
def walk_forward_tables(data: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    if not results.empty:
        ...
        results["fold_rank_type"] = np.where(results.get("fold_type", "").astype(str).str.contains("locked", case=False, na=False), "locked_holdout", "oos_fold")
...
def feature_evidence_table(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for feat in FEATURES:
        ...
        if not a.empty:
            ...
            decay = ", ".join(a[["horizon", "status"]].astype(str).drop_duplicates().apply(lambda x: f"{x.iloc[0]}:{x.iloc[1]}", axis=1).head(8).tolist()) if "horizon" in a.columns else "available"
        if not r.empty:
            status = "available"
...
def model_comparison_table(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if not garch.empty:
        for _, r in garch.iterrows():
            ...
            rows.append({"model": name, "status": role, "validation_metric": r.get("QLIKE", np.nan), "incremental_value": r.get("forecast_realized_corr", np.nan), "current_role": "MLE diagnostic only" if family == "mle" else "Grid model retained/diagnostic"})
    hmm = _df(data, "hmm_model_comparison")
    if not hmm.empty:
        for _, r in hmm.iterrows():
            n = r.get("n_states", "")
            rows.append({"model": f"HMM {n}-state", "status": "diagnostic", "validation_metric": r.get("BIC", np.nan), "incremental_value": r.get("state_interpretability", ""), "current_role": "HMM diagnostic; does not alter allocation"})
    inc = _df(data, "hmm_incremental_portfolio_results")
    if not inc.empty:
        for _, r in inc.iterrows():
            rows.append({"model": r.get("variant", "HMM overlay"), "status": "redundant_with_dual_trend", "validation_metric": r.get("Sharpe", np.nan), "incremental_value": r.get("net_CAGR", np.nan), "current_role": "research-only; no paper promotion"})
    rows.append({"model": "Dual trend", "status": "active", "validation_metric": "frozen config", "incremental_value": "current risk overlay", "current_role": "active exposure gate"})
    return pd.DataFrame(rows)
...
def evidence_reconciliation(kpis: dict[str, Any], warnings: pd.DataFrame) -> pd.DataFrame:
    rows = [
        {"evidence": "Deflated Sharpe", "value": kpis.get("exact_deflated_sharpe"), "interpretation": "strong if DSR p-value is low", "conflict": "does not eliminate CSCV PBO warning"},
        {"evidence": "CSCV PBO", "value": kpis.get("CSCV_PBO"), "interpretation": "elevated overfitting probability if above 0.5", "conflict": "coexists with strong OOS performance"},
        {"evidence": "Purged OOS", "value": kpis.get("mean_oos_sharpe"), "interpretation": "temporal OOS strength", "conflict": "official forward history still short"},
        {"evidence": "Locked Holdout", "value": kpis.get("locked_holdout_sharpe"), "interpretation": "strong holdout result", "conflict": "later development may reduce permanently untouched status"},
        {"evidence": "Parameter Stability", "value": kpis.get("parameter_stability_classification"), "interpretation": "broad plateau if stable_plateau/broad plateau", "conflict": "no new optimum selected"},
        {"evidence": "Alpha Decay", "value": "; ".join(warnings["warning"].astype(str).tolist()) if not warnings.empty else "none", "interpretation": "warnings remain visible", "conflict": "does not change frozen model automatically"},
    ]
    return pd.DataFrame(rows)


def integrity_table(data: dict[str, pd.DataFrame], src: pd.DataFrame, kpis: dict[str, Any], feature: pd.DataFrame) -> pd.DataFrame:
    missing_core = src[(~src["loaded"]) & src["data_key"].isin(["full_cscv_results", "pbo_distribution", "deflated_sharpe_exact", "purged_walk_forward_results", "parameter_stability_map"])]
    rows = [
        {"check": "research_files_load_safely", "status": "PASS" if missing_core.empty else "WARNING", "detail": ",".join(missing_core["source_file"].tolist())},
        ...
        {"check": "parameter_surface_available", "status": "PASS" if _df(data, "parameter_stability_map").shape[0] > 0 else "WARNING", "detail": _df(data, "parameter_stability_map").shape[0]},
        {"check": "missing_features_explicit", "status": "PASS" if "missing_data" in feature.get("availability", pd.Series(dtype=str)).astype(str).tolist() else "PASS", "detail": "feature evidence table includes missing-data reason"},
        {"check": "lifecycle_frozen_status", "status": "PASS" if "blocked" in str(kpis.get("lifecycle_status", "")).lower() or "frozen" in str(kpis.get("lifecycle_status", "")).lower() else "WARNING", "detail": kpis.get("lifecycle_status")},
    ]
    return pd.DataFrame(rows)
...
def build_research_bundle(data: dict[str, pd.DataFrame]) -> ResearchBundle:
    ...
    return ResearchBundle(kpis, anti, wf, param, feature, ic, models, registry, lifecycle, warnings, src, integrity, evidence, commentary, status)
