...
def drawdown_curve(perf: pd.DataFrame) -> pd.DataFrame:
    ...
    out["peak_date"] = (
        out.loc[eq.expanding().apply(lambda x: np.argmax(x), raw=True).fillna(0).astype(int).clip(upper=len(out) - 1).values, "date"].values
        if len(out)
        else pd.NaT
    )
...
def contribution_table(holdings: pd.DataFrame, returns: pd.DataFrame, cov: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, ticker in enumerate(tickers):
        ...
        daily_ret = (
            row.get("current_price", np.nan) / row.get("entry_price", np.nan) - 1 if pd.notna(row.get("entry_price", np.nan)) else np.nan
        )
...
def var_table(port_series: pd.Series, metrics: dict[str, Any]) -> pd.DataFrame:
    ...
    if obs >= 20:
        var95 = float(np.quantile(port_series, 0.05))
        var99 = float(np.quantile(port_series, 0.01))
    ...
    for name, value in [("Historical VaR 95%", var95), ("Historical VaR 99%", var99), ("CVaR 95%", cvar95), ("CVaR 99%", cvar99)]:
        rows.append(
            {"metric": name, "value": value, "methodology": method, "lookback": obs, "warmup_status": "WARMUP" if obs < 20 else "ACTIVE"}
        )
    return pd.DataFrame(rows)
...
def stress_tests(holdings: pd.DataFrame, contributions: pd.DataFrame, metrics: dict[str, Any]) -> pd.DataFrame:
    ...
    beta_map = (
        contributions.set_index("ticker")["beta_vs_spy"].to_dict()
        if not contributions.empty and "beta_vs_spy" in contributions.columns
        else {}
    )
    scenarios = []
    ...
    def add(name: str, shocks: dict[str, float], note: str):
        ...
        scenarios.append(
            {
                "scenario": name,
                "estimated_portfolio_loss": total,
                "largest_contributor": largest_contrib,
                "concentration_effect": loss_by.get(largest_contrib, np.nan),
                "diagnostic_note": note,
                "exposure_after_overlay_diagnostic": metrics.get("final_exposure", np.nan),
            }
        )
...
def tail_metrics(port_series: pd.Series, contributions: pd.DataFrame) -> dict[str, Any]:
    if port_series.empty:
        return {
            ...
            for k in [
                ...
                "tail_ratio",
                "gain_loss_asymmetry",
                "largest_negative_position_contribution",
            ]
        }
    ...
    largest_neg = (
        contributions.sort_values("drawdown_contribution").iloc[0].get("ticker")
        if not contributions.empty and "drawdown_contribution" in contributions.columns
        else "n/a"
    )
    return {
    ...
    rows = [
        {
            ...
            "current_value": metrics.get("final_exposure"),
            "status": "PASS"
            if pd.notna(metrics.get("final_exposure")) and metrics.get("final_exposure") <= MAX_EXPOSURE + 1e-9
            else "BREACH",
        },
        {
            "limit": "minimum_exposure",
            "configured_value": MIN_EXPOSURE,
            "current_value": metrics.get("final_exposure"),
            "status": "PASS"
            if pd.notna(metrics.get("final_exposure")) and metrics.get("final_exposure") >= MIN_EXPOSURE - 1e-9
            else "WARNING",
        },
        {
            "limit": "max_position_weight",
            "configured_value": MAX_POSITION_WEIGHT,
            "current_value": max_weight,
            "status": "PASS" if pd.notna(max_weight) and max_weight <= MAX_POSITION_WEIGHT + 1e-9 else "BREACH",
        },
        {
            "limit": "concentration_hhi_warning",
            "configured_value": CONCENTRATION_HHI_WARNING,
            "current_value": metrics.get("hhi"),
            "status": "PASS" if pd.notna(metrics.get("hhi")) and metrics.get("hhi") <= CONCENTRATION_HHI_WARNING else "WARNING",
        },
        {
            "limit": "vol_target_tolerance",
            "configured_value": TARGET_VOL,
            "current_value": metrics.get("realized_portfolio_volatility"),
            "status": "WARNING"
            if pd.notna(metrics.get("realized_portfolio_volatility")) and metrics.get("realized_portfolio_volatility") > TARGET_VOL * 1.5
            else "PASS",
        },
        {"limit": "turnover_threshold", "configured_value": TURNOVER_WARNING, "current_value": np.nan, "status": "NOT ENOUGH DATA"},
        {
            "limit": "stale_data_block",
            "configured_value": "fresh required",
            "current_value": metrics.get("source_date"),
            "status": "PASS" if metrics.get("source_date") else "NOT ENOUGH DATA",
        },
        {
            "limit": "drawdown_warning",
            "configured_value": DRAWDOWN_WARNING,
            "current_value": metrics.get("current_drawdown"),
            "status": "WARNING"
            if pd.notna(metrics.get("current_drawdown")) and metrics.get("current_drawdown") <= DRAWDOWN_WARNING
            else "PASS",
        },
        {
            "limit": "drawdown_hard_stop",
            "configured_value": DRAWDOWN_BREACH,
            "current_value": metrics.get("current_drawdown"),
            "status": "BREACH"
            if pd.notna(metrics.get("current_drawdown")) and metrics.get("current_drawdown") <= DRAWDOWN_BREACH
            else "PASS",
        },
        {
            "limit": "liquidity_adv_threshold",
            "configured_value": LIQUIDITY_ADV_THRESHOLD,
...
def build_risk_bundle(data: dict[str, pd.DataFrame], lookback: int = 126) -> RiskBundle:
    ...
    tick_returns = (
        asset_returns[[c for c in tickers if c in asset_returns.columns]].tail(lookback) if not asset_returns.empty else pd.DataFrame()
    )
    ...
    source_audit = pd.DataFrame(
        [
            ...
            {
                "source_file": "yahoo_ohlcv_price_cache",
                "namespace": "official_diagnostic_prices",
                "loaded": not asset_returns.empty,
                "row_count": len(asset_returns),
                "date_range": "local cache",
            },
        ]
    )
    checks = [
        {"check": "official_namespace_current_risk", "status": "PASS", "detail": "current holdings/performance from growth_official_*"},
        {
            ...
            "status": "PASS" if not holdings.empty and abs(float(holdings["weight"].sum()) - 1.0) < 1e-6 else "FAIL",
            "detail": str(holdings["weight"].sum() if not holdings.empty else np.nan),
        },
        {
            "check": "risk_contribution_sum",
            "status": "PASS"
            if not contributions.empty and abs(float(contributions["pct_total_portfolio_risk"].sum()) - 1.0) < 1e-5
            else "WARNING",
            "detail": str(contributions["pct_total_portfolio_risk"].sum() if not contributions.empty else np.nan),
        },
        {
            "check": "no_debug_leakage_current",
            "status": "PASS",
            "detail": "debug/reconstructed only allowed in separated comparison panels",
        },
        {"check": "missing_data_safe", "status": "PASS", "detail": "warmup and insufficient data states are explicit"},
        {
            "check": "three_d_inputs",
            "status": "PASS" if not corr.empty and not cov.empty else "WARNING",
            "detail": f"corr_shape={corr.shape}, cov_shape={cov.shape}",
        },
    ...
    return RiskBundle(
        holdings, tick_returns, cov, corr, metrics, contributions, vt, stress, limits, dd, tail, source_audit, integrity, status
    )


def risk_commentary(bundle: RiskBundle) -> str:
    contrib = (
        bundle.contributions.sort_values("pct_total_portfolio_risk", ascending=False) if not bundle.contributions.empty else pd.DataFrame()
    )
    ...
    floor = (
        bundle.metrics.get("final_exposure", np.nan) <= MIN_EXPOSURE + 1e-6
        if pd.notna(bundle.metrics.get("final_exposure", np.nan))
        else False
    )
    ...
    warmup = (
        "Official history remains in warmup."
        if bundle.metrics.get("history_observations", 0) < 20
        else "Official history has enough observations for portfolio-history diagnostics."
    )
