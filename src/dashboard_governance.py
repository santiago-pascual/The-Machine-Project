...
def _header(st, bundle) -> None:
    ...
    section_header(
        st,
        "Governance Terminal",
        "Model integrity, operational readiness, data quality, accounting, research gates and incident management. Read-only.",
        bundle.status,
    )
    cols = st.columns(4)
    with cols[0]:
        metric_card(st, "Active Version", str(k.get("active_model_version")), str(k.get("lifecycle_classification")), badge="frozen")
    with cols[1]:
        metric_card(st, "Official Date", str(k.get("latest_official_date")), f"last run {k.get('latest_successful_run')}", badge="official")
    with cols[2]:
        metric_card(
            st, "Integrity / Market", f"{k.get('integrity_status')} / {k.get('market_data_status')}", "official governance", badge="paper"
        )
    with cols[3]:
        metric_card(
            st, "Real Capital", str(k.get("real_capital_status")), str(k.get("broker_orders_status")), state="warning", badge="blocked"
        )
...
def _real_capital(st, bundle) -> None:
    ...
    with c1:
        metric_card(st, "Passed Gates", f"{passed}/{total}", "configured governance gates")
    with c2:
        metric_card(st, "Classification", "REAL CAPITAL BLOCKED", "no promotion automation", state="warning")
    with c3:
        metric_card(st, "Broker Orders", "DISABLED", "read-only dashboard", state="neutral")
...
def _freshness(st, bundle) -> None:
    ...
    alert_box(
        st,
        "Primary provider is Yahoo/cache. Secondary provider absence blocks real capital but does not block official paper when Yahoo is fresh.",
        "warning",
    )
...
def _history_sources(st, bundle) -> None:
    ...
    if not hist.empty and "date" in hist.columns and "current_status" in hist.columns:
        fig = px.scatter(
            ...
            x="date",
            y="current_status",
            color="promotion_status" if "promotion_status" in hist.columns else None,
...
def render_governance_terminal(st, data: dict[str, pd.DataFrame]) -> None:
    ...
    with tabs[0]:
        _scoreboard(st, bundle)
    with tabs[1]:
        _real_capital(st, bundle)
    with tabs[2]:
        _lifecycle_version(st, bundle)
    with tabs[3]:
        _pipeline(st, bundle)
    with tabs[4]:
        _freshness(st, bundle)
    with tabs[5]:
        _incidents_backups(st, bundle)
    with tabs[6]:
        _accounting_research(st, bundle)
    with tabs[7]:
        _history_sources(st, bundle)
