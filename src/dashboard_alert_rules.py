...
def generate_alerts(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    dd = (
        float(pd.to_numeric(pd.Series([perf.get("current_drawdown", perf.get("max_drawdown", np.nan))]), errors="coerce").iloc[0])
        if not perf.empty
        else np.nan
    )
    ...
    if not actions.empty and "action" in actions.columns:
        for action, sev, desc in [
            ("BUY", "INFO", "New holding enters"),
            ("SELL", "NOTICE", "Holding exits"),
            ("INCREASE", "NOTICE", "Weight increases significantly"),
            ("REDUCE", "NOTICE", "Weight reduces significantly"),
        ]:
        ...
        if pd.notna(turnover) and turnover > 0.50:
            _add(
                ...
                "WARNING",
                "Portfolio",
                "Portfolio turnover unusually high",
                turnover,
                "0.50",
    ...
    if costs.empty:
        ...
        if total_cost > 500:
            _add(
                ...
                "WARNING",
                "Execution",
                "Estimated costs unusually high",
                float(total_cost),
                "$500",
                "growth_official_estimated_cost_ledger.csv",
            )
    if pd.notna(exposure) and pd.notna(vol) and exposure > 0.55 and vol > 0.22:
        _add(
            ...
            "NOTICE",
            "Execution",
            "Execution/risk stack elevated",
            f"exposure={exposure}, vol={vol}",
            "exposure>0.55 and vol>0.22",
            "growth_official_paper_performance.csv",
        )
    ...
    if md.empty:
        _add(
            ...
            "CRITICAL",
            "Market Data",
            "Missing market data integrity file",
            "missing",
            "file required",
        ...
        if "is_fresh" in md.columns and md["is_fresh"].astype(str).str.lower().isin(["false", "0"]).any():
            _add(
                ...
                "Market Data",
                "Stale market data",
                "one or more tickers stale",
                "all fresh",
                "official_market_data_integrity.csv",
            )
        for col, desc in [
            ("adjusted_close_available", "Adjusted prices unavailable"),
            ("volume_available", "Volume unavailable"),
            ("exists", "Missing OHLCV cache"),
        ]:
    ...
    if not monitor.empty:
        ...
        if "warmup" in gov.lower():
            _add(
                ...
                "NOTICE",
                "Governance",
                "Forward history too short",
                gov,
                "minimum paper history",
                "growth_official_paper_monitor.csv",
            )
    ...
    if not oos.empty and any(x in str(oos.get("classification", "")).lower() for x in ["fail", "unstable"]):
        _add(
            ...
            "WARNING",
            "Research",
            "Walk-forward deterioration",
            oos.get("classification"),
            "passes_oos",
    ...
    recon = (
        _latest_row(data, "official_accounting_reconciliation") if "official_accounting_reconciliation" in data else pd.Series(dtype=object)
    )
    dup = _latest_row(data, "official_cost_duplication_audit")
    if not recon.empty:
        for col, desc in [
            ("gross_identity_pass", "Gross ledger mismatch"),
            ("net_identity_pass", "Net/Gross mismatch"),
            ("weights_sum_to_one", "Position reconciliation failure"),
        ]:
    ...
    if not dup.empty and float(pd.to_numeric(pd.Series([dup.get("duplicate_cost_rows", 0)]), errors="coerce").iloc[0]) > 0:
        _add(
            ...
            "Accounting",
            "Duplicate costs",
            dup.get("duplicate_cost_rows"),
            "0",
            "official_cost_duplication_audit.csv",
        )

    # Dashboard module alerts
    for key, module, source in [
        ("decision_engine_integrity", "Decision Engine", "decision_engine_integrity.csv"),
        ("historical_replay_integrity", "Historical Replay", "historical_replay_integrity.csv"),
        ("quant_lab_surface_integrity", "Dashboard", "quant_lab_surface_integrity.csv"),
    ]:
