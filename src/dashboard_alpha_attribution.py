...
def render_alpha_attribution(st, data: dict[str, pd.DataFrame]) -> None:
    ...
    st.caption(
        "Strictly analytical. Additive attribution is reported in log-return space; unavailable marginal effects remain in Residual."
    )
    ...
    cols = st.columns(4)
    with cols[0]:
        metric_card(st, "Status", result["status"], "read-only")
    with cols[1]:
        metric_card(st, "Total Log Return", f"{float(recon.iloc[-1]['total_return']):.4f}" if not recon.empty else "n/a")
    with cols[2]:
        metric_card(st, "Recon Diff", f"{float(recon.iloc[-1]['difference']):.2e}" if not recon.empty else "n/a")
    with cols[3]:
        metric_card(st, "Recon", str(recon.iloc[-1].get("reconciliation_pass", "n/a")) if not recon.empty else "n/a")
    if not attr.empty:
        fig = px.bar(
            attr,
            x="component",
            y="alpha_contribution",
            color="alpha_contribution",
            color_continuous_scale="Oranges",
            title="Alpha Attribution Waterfall Inputs",
        )
        fig = apply_plotly_layout(fig, "Alpha Attribution Components")
        st.plotly_chart(fig, width="stretch")
        pie = px.pie(
            attr[attr["absolute_contribution"] > 0], names="component", values="absolute_contribution", title="Absolute Attribution Share"
        )
