...
def markdown_report(bundle: ReportBundle) -> str:
    ...
    lines = [
        ...
        _kv_md(
            ...
            {k: fmt_pct(v) if k in {"CAGR", "Max Drawdown", "Volatility"} else fmt_num(v) for k, v in stats.items()},
        ),
        "## 3. Portfolio",
...
def html_report(bundle: ReportBundle) -> str:
    ...
    alerts = bundle.sections["alerts"]["active"]
    stat_rows = [
        {"metric": k, "value": fmt_pct(v) if k in {"CAGR", "Max Drawdown", "Volatility"} else fmt_num(v)} for k, v in perf["stats"].items()
    ]
    kpis = "".join(
        f"<div class='card'><div class='label'>{esc(k.replace('_', ' ').title())}</div><div class='value'>{esc(v)}</div></div>"
        for k, v in ex.items()
    )
