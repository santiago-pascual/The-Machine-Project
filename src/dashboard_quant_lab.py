...
def _source_rows(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    keys = [
        "official_state", "official_performance", "current_features", "current_raw_target_features", "current_allocation",
        "vol_fresh", "parameter_stability", "parameter_stability_map", "hmm_oos", "hmm_model_comparison",
        "official_benchmark_daily", "canonical_price_history", "factor_correlation_matrix", "garch_model_comparison",
    ]
    rows = []
    for key in keys:
        ...
        rows.append({
            ...
            "active_model_impact": "active" if key in {"official_state", "current_features", "current_raw_target_features", "vol_fresh"} else "diagnostic only",
        })
...
def _surface_plan(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    checks = [
        ...
        ("volatility_target_surface", "growth_volatility_targeting_fresh.csv", "fresh vol target audit", not data.get("vol_fresh", pd.DataFrame()).empty),
        ("parameter_stability_surface", "parameter_stability_map.csv", "Phase 92 grid", not data.get("parameter_stability", pd.DataFrame()).empty or not data.get("parameter_stability_map", pd.DataFrame()).empty),
        ("black_litterman_lab", "black_litterman_*", "BL grids", False),
        ("hmm_regime_space", "hmm_out_of_sample_results.csv", "HMM diagnostics", not data.get("hmm_oos", pd.DataFrame()).empty or not data.get("hmm_model_comparison", pd.DataFrame()).empty),
        ("feature_space_3d", "current_growth_features.csv", "current feature vectors", not data.get("current_features", pd.DataFrame()).empty),
    ...
    return pd.DataFrame([{"surface": n, "source": s, "required_input": r, "status": "available" if ok else "warning", "detail": "input present" if ok else "input missing or diagnostic unavailable"} for n, s, r, ok in checks])
...
def build_quant_lab_audits(data: dict[str, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, str]:
    ...
    perf = pd.DataFrame([
        ...
        {"item": "cache_policy", "status": "PASS", "detail": "st.cache_data wrappers for price matrices and Monte Carlo when Streamlit is available"},
...
def render_header(st, data: dict[str, pd.DataFrame]) -> None:
    ...
    cols = st.columns(4)
    with cols[0]: metric_card(st, "Active Model", MODEL_VERSION, VARIANT, badge="READ ONLY")
    with cols[1]: metric_card(st, "Latest Market Date", latest_market_date(data), "official data scope")
    with cols[2]: metric_card(st, "Current Holdings", ", ".join(tickers) if tickers else "unavailable")
    with cols[3]: metric_card(st, "Exposure", fmt_pct(exposure), lifecycle, badge="diagnostic")
    alert_box(st, "Diagnostic visualization only. Some models shown here are research-only and do not influence the official Growth allocation.", "warning")
...
def _render_section(st, data: dict[str, pd.DataFrame], section: str) -> dict[str, object]:
    ...
    if section == "Portfolio Geometry":
        ...
        result = {"surface": "correlation_covariance", "status": "available" if r1.get("status") == "available" or r2.get("status") == "available" else "warning", "detail": f"{r1.get('status')}/{r2.get('status')}"}
        ...
        result = {"surface": "feature_space", "status": "available" if r1.get("status") == "available" or r2.get("status") == "available" else "warning", "detail": f"{r1.get('detail')}; {r2.get('detail')}"}
...
def render_quant_lab(st, data: dict[str, pd.DataFrame]) -> None:
    ...
    st.caption(f"Render status: {status_badge(str(result.get('status', 'unknown')), str(result.get('status', 'diagnostic')))} · {result.get('render_seconds', 'n/a')}s", unsafe_allow_html=True)
