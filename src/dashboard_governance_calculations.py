...
def _status_from_bool(value: Any, default="NOT AVAILABLE") -> str:
    if isinstance(value, str):
        v = value.lower()
        if (
            v in {"pass", "passed", "ok", "true", "ready", "healthy"}
            or "ready" in v
            or "strong_oos" in v
            or "stable_plateau" in v
            or "exact_rebalance_parity" in v
        ):
            return "PASS"
        if "warmup" in v:
            return "WARMUP"
    ...
    rows = [
        {
            "date": "2026-06-29",
            "phase": "Freshness",
            "issue": "stale forecast_history prevented latest growth paper date",
            "severity": "HIGH",
            "affected_files": "forecast_history.csv,current_growth_features.csv",
            "action_taken": "freshness gate added before growth paper",
            "backup_created": "not_required",
            "repair_result": "fixed",
            "validation_result": "data_freshness_audit",
            "current_status": "closed",
        },
        {
            "date": "2026-07-06",
            "phase": "Volatility",
            "issue": "stale volatility source pinned exposure to minimum floor",
            "severity": "HIGH",
            "affected_files": "growth_volatility_targeting_daily_returns.csv",
            "action_taken": "fresh OHLCV volatility source for paper",
            "backup_created": "not_required",
            "repair_result": "fixed",
            "validation_result": "growth_full_integrity_audit",
            "current_status": "closed",
        },
        {
            "date": "2026-07-10",
            "phase": "Official",
            "issue": "debug/reconstructed history separated from official paper namespace",
            "severity": "MEDIUM",
            "affected_files": "growth_candidate_paper_*.csv,growth_official_paper_*.csv",
            "action_taken": "official namespace reset",
            "backup_created": "archived debug history",
            "repair_result": "fixed",
            "validation_result": "official_paper_integrity_status.csv",
            "current_status": "closed",
        },
        {
            "date": "2026-07-13",
            "phase": "Accounting",
            "issue": "official t+1 accounting and initial cost validation",
            "severity": "MEDIUM",
            "affected_files": "growth_official_paper_performance.csv,growth_official_estimated_cost_ledger.csv",
            "action_taken": "accounting audit and cost duplication controls",
            "backup_created": "official_accounting_backup_20260713_090128",
            "repair_result": "warning_only",
            "validation_result": "official_forward_accounting_audit.csv",
            "current_status": "monitored",
        },
        {
            "date": "2026-07-13",
            "phase": "Dashboard",
            "issue": "dashboard source namespace and benchmark chart mismatch repairs",
            "severity": "MEDIUM",
            "affected_files": "dashboard_app.py,growth_official_benchmark_*.csv",
            "action_taken": "official scope chart source repair",
            "backup_created": "not_required",
            "repair_result": "fixed",
            "validation_result": "benchmark_chart_reconciliation.csv",
            "current_status": "closed",
        },
    ]
...
def build_kpis(data: dict[str, pd.DataFrame]) -> dict[str, Any]:
    ...
    frozen_match = (
        frozen[frozen.get("registry_name", pd.Series(dtype=str)).astype(str).str.contains("growth_champion_final_v1_0", na=False)]
        if not frozen.empty
        else pd.DataFrame()
    )
    lifecycle_status = (
        frozen_match.iloc[-1].get("status", "operational_paper_production_real_capital_blocked")
        if not frozen_match.empty
        else "operational_paper_production_real_capital_blocked"
    )
    return {
        ...
        "parameter_hash": (
            frozen["parameter_set_hash"].dropna().iloc[-1]
            if (not frozen.empty and "parameter_set_hash" in frozen.columns and not frozen["parameter_set_hash"].dropna().empty)
            else np.nan
        ),
    }
...
def scoreboard(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "source_file": GOVERNANCE_SOURCES.get(source, source),
                "reason": reason,
                "blocking": blocking,
            }
        )
    ...
    sys_report = (
        Path("growth_system_integrity_report.txt").read_text(encoding="utf-8", errors="ignore")
        if Path("growth_system_integrity_report.txt").exists()
        else ""
    )
    add(
        "Pipeline integrity",
        "PASS" if "Pipeline integrity: PASS" in sys_report else "WARNING",
        "growth_system_integrity_report",
        "growth system report",
        "non-blocking for paper",
    )
    add(
        "Historical replay",
        "PASS" if "Historical replay: PASS" in sys_report else "WARNING",
        "growth_system_integrity_report",
    ...
    add(
        "Daily paper",
        _status_from_bool(_latest(_df(data, "official_daily_status")).get("status", "")),
        "official_daily_status",
        "official daily run status",
        "blocking if fail",
    )
    add(
        "Dashboard integrity",
        "PASS" if "Dashboard: PASS" in sys_report else "WARNING",
        "growth_system_integrity_report",
        "dashboard report plus py_compile",
        "non-blocking",
    )
    add(
        "Trade reconciliation",
        "PASS" if "Trade reconciliation: PASS" in sys_report else "WARNING",
        "growth_system_integrity_report",
    ...
    add(
        ...
        "PASS" if "Volatility targeting: PASS" in sys_report else "WARNING",
        "growth_system_integrity_report",
        "fresh volatility targeting validation",
        "blocking if stale",
    )
    add(
        "Market data freshness",
        _status_from_bool(_latest(_df(data, "official_market_data_governance")).get("classification", "")),
        "official_market_data_governance",
        _latest(_df(data, "official_market_data_governance")).get("reason", ""),
        "blocking for real capital",
    )
    add(
        "Secondary data confirmation",
        _status_from_bool(_latest(_df(data, "market_data_governance")).get("classification", "")),
        "market_data_governance",
        _latest(_df(data, "market_data_governance")).get("reason", ""),
        "real-capital blocker",
    )
    add(
        "Official accounting",
        "PASS" if bool(_latest(_df(data, "official_accounting_audit")).get("accounting_pass", False)) else "FAIL",
        "official_accounting_audit",
        _latest(_df(data, "official_accounting_audit")).get("governance", ""),
        "blocking if fail",
    )
    add(
        "Rebalance parity",
        _status_from_bool(_latest(_df(data, "growth_rebalance_parity_governance")).get("classification", "")),
        "growth_rebalance_parity_governance",
        _latest(_df(data, "growth_rebalance_parity_governance")).get("notes", ""),
        "blocking if fail",
    )
    add(
        "Canonical history",
        _status_from_bool(_latest(_df(data, "growth_canonical_governance")).get("classification", "")),
        "growth_canonical_governance",
        _latest(_df(data, "growth_canonical_governance")).get("warning", ""),
        "non-blocking warning",
    )
    add(
        "Anti-overfitting",
        _status_from_bool(_latest(_df(data, "anti_overfitting_governance")).get("classification", "")),
        "anti_overfitting_governance",
        _latest(_df(data, "anti_overfitting_governance")).get("reason", ""),
        "real-capital blocker",
    )
    add(
        "Purged walk-forward",
        _status_from_bool(_latest(_df(data, "out_of_sample_governance")).get("classification", "")),
        "out_of_sample_governance",
        _latest(_df(data, "out_of_sample_governance")).get("governance_note", ""),
        "non-blocking",
    )
    add(
        "Parameter stability",
        _status_from_bool(_latest(_df(data, "parameter_governance")).get("classification", "")),
        "parameter_governance",
        _latest(_df(data, "parameter_governance")).get("reason", ""),
        "non-blocking",
    )
    add(
        "Live health",
        _status_from_bool(_latest(_df(data, "growth_paper_governance_history")).get("current_status", "")),
        "growth_paper_governance_history",
        _latest(_df(data, "growth_paper_governance_history")).get("reason", ""),
        "paper warmup",
    )
...
def real_capital_gates(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    rows = [
        (
            "Official forward history minimum",
            "configured governance minimum",
            f"paper_days={paper.get('paper_days', 'unavailable')}",
            False,
            "growth_paper_governance_history.csv",
        ),
        (
            "CSCV PBO threshold",
            "configured/governance acceptable PBO",
            f"PBO={anti.get('CSCV_PBO', np.nan):.3f}",
            False,
            "anti_overfitting_governance.csv",
        ),
        (
            "DSR threshold",
            "configured/governance DSR pass",
            f"DSR p={anti.get('DSR_p_value', np.nan):.4f}",
            True,
            "anti_overfitting_governance.csv",
        ),
        ...
        (
            "Parameter stability",
            "stable_plateau",
            param.get("classification", ""),
            str(param.get("classification", "")).lower() == "stable_plateau",
            "parameter_governance.csv",
        ),
        ...
        (
            "Execution slippage validation",
            "live fill slippage validated",
            "estimated only",
            False,
            "phase110_execution_terminal_report.txt",
        ),
        (
            "Accounting integrity",
            "accounting_pass true",
            acct.get("accounting_pass", False),
            bool(acct.get("accounting_pass", False)),
            "official_forward_accounting_audit.csv",
        ),
        ("Operational redundancy", "redundant monitored process", "single local machine", False, "governance terminal"),
        (
            "Maximum acceptable drawdown",
            "governance threshold",
            f"current={paper.get('drawdown', np.nan)}",
            True,
            "growth_paper_governance_history.csv",
        ),
        ...
        [
            {
                ...
                "current_state": cur,
                "passed": passed,
                "blocker_reason": "" if passed else "real-capital blocker",
                "evidence_source": src,
            }
            for g, req, cur, passed, src in rows
        ]
    )
        ...
        rows.append(
            {
                "stage": stage,
                "current": current,
                "date": "2026-07-10" if current else "",
                "evidence_required": "phase-specific governance gates",
                "evidence_achieved": "yes" if current else "partial/not started",
                "approval_status": "approved for paper only" if current else "not approved",
                "rollback_condition": "integrity fail, stale data, accounting fail, or governance block",
            }
        )
    return pd.DataFrame(rows)
...
def version_table(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    files = [
        "growth_candidate_paper_config.json",
    ...
    rows = [
        {"item": "model_version", "value": "growth_champion_final_v1_0_frozen", "status": "PASS"},
        {
            "item": "configuration_hash",
            "value": frozen.get("parameter_set_hash", "missing"),
            "status": "WARNING" if pd.isna(frozen.get("parameter_set_hash", np.nan)) else "PASS",
        },
    ]
    for f in files:
...
def pipeline_table(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for node, key, file in PIPELINE_ARTIFACTS:
        ...
        status = (
            row.get("integrity_status", row.get("status", row.get("classification", "PASS" if exists else "NOT AVAILABLE")))
            if exists
            else "NOT AVAILABLE"
        )
        rows.append(
            {
                "node": node,
                "status": _status_from_bool(status),
                "latest_date": date,
                "row_count": len(df),
                "source_file": file,
...
def accounting_table(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    audit = _df(data, "official_accounting_audit")
    recon = _df(data, "official_accounting_reconciliation")
    rows = []
    if not audit.empty:
        r = audit.iloc[-1]
        rows += [
    ...
    if not recon.empty:
        r = recon.iloc[-1]
        rows += [
            ("weights sum to 1", r.get("weights_sum_to_one", False), "official_accounting_reconciliation.csv"),
            ("holdings + cash = gross equity", r.get("holdings_cash_identity_pass", False), "official_accounting_reconciliation.csv"),
            ("gross identity", r.get("gross_identity_pass", False), "official_accounting_reconciliation.csv"),
            ("net identity", r.get("net_identity_pass", False), "official_accounting_reconciliation.csv"),
        ]
...
def research_table(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    oos = _latest(_df(data, "out_of_sample_governance"))
    param = _latest(_df(data, "parameter_governance"))
    alpha = _latest(_df(data, "alpha_decay_governance"))
    rows = [
        {
            "metric": "all_time_trials",
            "value": anti.get("all_time_trials", np.nan),
            "status": "WARNING",
            "evidence": "anti_overfitting_governance.csv",
        },
        {
            "metric": "effective_independent_trials",
            "value": anti.get("effective_independent_trials", np.nan),
            "status": "PASS",
            "evidence": "anti_overfitting_governance.csv",
        },
        {
            "metric": "PBO",
            "value": anti.get("CSCV_PBO", np.nan),
            "status": "WARNING",
            "evidence": "strong DSR/OOS does not override elevated PBO",
        },
        ...
        {
            "metric": "parameter_robustness",
            "value": param.get("classification", ""),
            "status": "PASS",
            "evidence": "parameter_governance.csv",
        },
...
def build_governance_bundle(data: dict[str, pd.DataFrame]) -> GovernanceBundle:
    ...
    integrity = pd.DataFrame(
        [
            {
                "check": "lifecycle_matches_frozen_registry",
                "status": "PASS" if "frozen" in str(kpis.get("active_model_version", "")) else "WARNING",
                "detail": kpis.get("active_model_version"),
            },
            {
                "check": "real_capital_blocked",
                "status": "PASS" if kpis.get("real_capital_status") == "real_capital_blocked" else "FAIL",
                "detail": kpis.get("real_capital_status"),
            },
            {"check": "technical_pass_does_not_hide_warnings", "status": "PASS", "detail": "persistent warnings panel always visible"},
            {
                "check": "pipeline_dates_reconcile",
                "status": "WARNING" if fresh["freshness_status"].eq("WARNING").any() else "PASS",
                "detail": "see freshness table",
            },
    ...
    if integrity["status"].eq("FAIL").any():
        status = "governance_terminal_fail"
    elif integrity["status"].eq("WARNING").any() or score["status"].isin(["WARNING", "BLOCKED", "WARMUP"]).any() or not warns.empty:
        status = "governance_terminal_warning"
