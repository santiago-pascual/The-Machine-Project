...
def decile_analysis(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    work["rank_percentile"] = 1 - (numeric(work["raw_target_rank"]) - numeric(work["raw_target_rank"]).min()) / max(numeric(work["raw_target_rank"]).max() - numeric(work["raw_target_rank"]).min(), 1)
    work["forecast_decile"] = pd.qcut(work["rank_percentile"].rank(method="first"), 10, labels=False, duplicates="drop") + 1
    out = work.groupby("forecast_decile").agg(
...
def ranking_analysis(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for n in [10, 20, 50]:
        ...
        rows.append({"rank_bucket": f"top_{n}", "observations": len(sub), "average_asset_return": numeric(sub.get("asset_return", pd.Series(dtype=float))).mean(), "hit_rate": (numeric(sub.get("asset_return", pd.Series(dtype=float))) > 0).mean() if len(sub) else np.nan})
    by_rank = work.groupby("raw_target_rank").agg(observations=("ticker", "count"), average_return=("asset_return", "mean")).reset_index().head(50)
    rows.extend(by_rank.assign(rank_bucket=lambda d: "rank_" + d["raw_target_rank"].astype(str)).rename(columns={"average_return": "average_asset_return"}).to_dict("records"))
...
def sizing_analysis(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for d, g in work.groupby("date"):
        ...
        diffs.append(diff)
        rows.append({"date": d, "selected_count": len(g), "exposure": exposure, "current_weighted_return": current_return, "equal_weight_return": equal_return, "sizing_alpha": diff})
...
def turnover_analysis(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame([{
        ...
        "turnover_return_correlation": work["turnover"].corr(numeric(work.get("gross_daily_return", pd.Series(dtype=float)))) if len(work) > 2 else np.nan,
        "high_turnover_days": int((work["turnover"] > work["turnover"].quantile(0.9)).sum()) if len(work) > 10 else int((work["turnover"] > 0).sum()),
    }])
...
def holding_period_analysis(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    return work.groupby("ticker").agg(
        observations=("ticker", "count"),
        average_holding_days=("holding_days", "mean"),
        average_return=("asset_return", "mean"),
        hit_rate=("winner", "mean"),
        total_contribution=("trade_contribution", "sum"),
    ).reset_index().sort_values("total_contribution", ascending=True)


def cost_attribution(daily: pd.DataFrame, official_costs: pd.DataFrame) -> tuple[pd.DataFrame, float]:
    rows = []
    hist_cost_return = numeric(daily.get("net_daily_return", pd.Series(dtype=float))).fillna(0) - numeric(daily.get("gross_daily_return", pd.Series(dtype=float))).fillna(0) if not daily.empty else pd.Series(dtype=float)
    ...
    official_total_cost = numeric(official_costs.get("estimated_total_cost", pd.Series(dtype=float))).sum() if not official_costs.empty else 0.0
    rows.append({"source": "official_forward_estimated_costs", "cost_alpha": -official_total_cost / INITIAL_CAPITAL, "notes": "official estimated reporting-only costs / initial capital"})
