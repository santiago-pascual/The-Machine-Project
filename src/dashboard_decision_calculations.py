        ...
        rows.append(
            {
                ...
                "namespace": "official_current_decision",
            }
        )
            ...
            node_rows.append(
                {
                    ...
                    "label": label,
                    "stage": label,
                    "count": row.get("passed_count", np.nan),
                    "retained_pct": row.get("retained_pct", np.nan),
            ...
            node_rows.append(
                {
                    "id": i,
                    "label": label,
                    "stage": "Final Holding" if label != "CASH" else "Cash",
                    "count": 1,
                    "retained_pct": 1,
                    "source_file": "growth_official_paper_state.csv",
                }
            )
    ...
    if not funnel.empty:
            ...
            links.append(
                {
                    "source": i,
                    "target": i + 1,
                    "value": float(next_row.get("input_count", prev_row.get("passed_count", 0)))
                    if pd.notna(next_row.get("input_count", np.nan))
                    else float(prev_row.get("passed_count", 0)),
        ...
        for t in tickers:
            links.append(
                {
                    "source": final_idx,
                    "target": labels.index(t),
                    "value": 1,
                    "stage": "Final Portfolio -> holding",
                    "excluded_count": 0,
                    "retained_pct": 1,
                    "reason_summary": f"Final holding {t}",
                }
            )
        links.append(
            {
                "source": final_idx,
                "target": labels.index("CASH"),
                "value": 1,
                "stage": "Final Portfolio -> cash",
                "excluded_count": 0,
                "retained_pct": 1,
                "reason_summary": "Cash sleeve",
            }
        )
...
def build_kpis(
    data: dict[str, pd.DataFrame], funnel: pd.DataFrame, selected: pd.DataFrame, rejected: pd.DataFrame, pending: pd.DataFrame
) -> dict[str, Any]:
    ...
    return {
        "active_model_version": state.iloc[-1].get(
            "growth_model_version", state.iloc[-1].get("model_version", "growth_champion_final_v1_0_frozen")
        )
        if not state.empty
        else "growth_champion_final_v1_0_frozen",
        ...
        "signal_date": state.iloc[-1].get(
            "signal_date", actions.iloc[-1].get("signal_date", "unavailable") if not actions.empty else "unavailable"
        )
        if not state.empty
        else "unavailable",
        ...
        "next_rebalance": schedule.iloc[-1].get(
            "next_rebalance_date", state.iloc[-1].get("next_rebalance_date", "unavailable") if not state.empty else "unavailable"
        )
        if not schedule.empty
        else "unavailable",
        "sessions_until_next_rebalance": 5 - int(schedule.iloc[-1].get("sessions_since_last_rebalance", 0))
        if not schedule.empty and pd.notna(schedule.iloc[-1].get("sessions_since_last_rebalance", np.nan))
        else "unavailable",
        ...
        "pending_enters": ",".join(
            pending[pending.get("pending_signal_type", "").astype(str).eq("PENDING_ENTER")]
...
def decision_tree_table(selected: pd.DataFrame, rejected: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in base.iterrows():
        ...
        rules = [
            ...
            (
                "Missing OHLCV <= threshold",
                "missing ohlcv" not in str(row.get("exact_exclusion_reason", row.get("final_exclusion_reason", ""))).lower(),
            ),
...
def attrition_table(funnel: pd.DataFrame, rejected: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.concat(
        [stage.rename(columns={"excluded_count": "count"})[["stage", "count", "retained_pct", "source_file"]], reason_counts],
        ignore_index=True,
        sort=False,
    )
        ...
        rows.append(
            {
                ...
                "score": float(val),
                "source_column": col,
                "note": "Diagnostic profile - not an additional allocation model.",
            }
        )
    return pd.DataFrame(rows)
        ...
        rows.append(
            {
                ...
                "file_or_module": file,
                "latest_date": date,
                "status": "PASS" if not df.empty else "NOT AVAILABLE",
                "input_output_rows": len(df),
                "exact_or_proxy": "exact" if scope in {"exact", "official", "official_current"} else "observed_candidate",
                "scope": scope,
            }
        )
    return pd.DataFrame(rows)
...
def integrity_table(
    kpis: dict[str, Any], selected: pd.DataFrame, state: pd.DataFrame, funnel: pd.DataFrame, rejected: pd.DataFrame
) -> pd.DataFrame:
    official = (
        set(state[state.get("ticker", "").astype(str).str.upper().ne("CASH")].get("ticker", pd.Series(dtype=str)).astype(str).str.upper())
        if not state.empty
        else set()
    )
    ...
    rows = [
        {
            "check": "final_holdings_equal_official_portfolio",
            "status": "PASS" if official == explained else "FAIL",
            "detail": f"official={sorted(official)}, explained={sorted(explained)}",
        },
        {
            "check": "sankey_counts_available",
            "status": "PASS" if not funnel.empty and funnel.get("passed_count", pd.Series(dtype=float)).notna().all() else "WARNING",
            "detail": len(funnel),
        },
        {
            "check": "rejected_candidates_have_reasons",
            "status": "PASS"
            if rejected.empty or rejected.get("exact_exclusion_reason", pd.Series(dtype=str)).fillna("").astype(str).str.len().gt(0).all()
            else "WARNING",
            "detail": len(rejected),
        },
        {
            "check": "no_fabricated_feature_contributions",
            "status": "PASS",
            "detail": "feature values shown; exact marginal contribution labeled unavailable unless stored",
        },
        {
            "check": "official_current_namespace",
            "status": "PASS",
            "detail": "debug/reconstructed sources not used in current decision view",
        },
...
def build_decision_bundle(data: dict[str, pd.DataFrame]) -> DecisionBundle:
    ...
    missing = pd.DataFrame(
        [
            {
                "item": "exact feature marginal contribution weights",
                "status": "not_stored",
                "note": "Feature value available; exact marginal contribution not stored.",
            }
        ]
    )
    status = "decision_engine_pass"
    if integ["status"].eq("FAIL").any():
        status = "decision_engine_fail"
    elif integ["status"].eq("WARNING").any() or not missing.empty:
        status = "decision_engine_warning"
