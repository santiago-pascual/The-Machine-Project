...
def render_monte_carlo_lab(st, data: dict[str, pd.DataFrame]) -> dict[str, object]:
    ...
    tickers = (
        holdings[~holdings.get("ticker", "").astype(str).str.upper().eq("CASH")]["ticker"].astype(str).str.upper().tolist()
    ...
    initial = (
        float(numeric(holdings.get("gross_portfolio_value", pd.Series([100000]))).dropna().iloc[-1])
        if "gross_portfolio_value" in holdings.columns
        and numeric(holdings.get("gross_portfolio_value", pd.Series(dtype=float))).notna().any()
        else 100000.0
    )
    ...
    cols = st.columns(4)
    with cols[0]:
        metric_card(st, "Expected Terminal", fmt_money(np.mean(terminal)))
    with cols[1]:
        metric_card(st, "Probability Loss", fmt_pct(np.mean(terminal < initial)))
    with cols[2]:
        metric_card(st, "VaR 95", fmt_pct(var95))
    with cols[3]:
        metric_card(st, "CVaR 95", fmt_pct(cvar95))
