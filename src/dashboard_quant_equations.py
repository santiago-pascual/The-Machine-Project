...
EQUATIONS = {
    "Expected return": (
        r"r_e = \frac{P_{target}/P_0 - 1}{T}",
        "target price, current price, time-to-target",
        "expected_returns_model.py",
        "active raw target diagnostic",
    ),
    "Volatility targeting": (
        r"E_t = \min(\max(\sigma^*/\hat\sigma_t, 0.40), 0.60, C_{trend})",
        "target volatility, realized portfolio volatility, exposure caps",
        "growth_volatility_targeting_fresh.csv",
        "active official risk overlay",
    ),
    "Sharpe": (
        r"S = \frac{\bar r - r_f}{\sigma_r}\sqrt{252}",
        "mean return, risk-free rate, volatility",
        "performance reports",
        "diagnostic/governance",
    ),
    "Sortino": (
        r"Sortino = \frac{\bar r - r_f}{\sigma_{down}}\sqrt{252}",
        "downside deviation",
        "performance reports",
        "diagnostic/governance",
    ),
    ...
    "VaR / CVaR": (
        r"VaR_\alpha = q_\alpha(r), \quad CVaR_\alpha = E[r | r \le VaR_\alpha]",
        "return quantiles and tail mean",
        "risk/monte carlo",
        "diagnostic",
    ),
    ...
    "Black-Litterman": (
        r"\mu_{BL} = \pi + \tau\Sigma P^T(P\tau\Sigma P^T + \Omega)^{-1}(q-P\pi)",
        "prior, covariance, views, uncertainty",
    ...
    "EGARCH": (
        r"\log\sigma_t^2 = \omega + \beta\log\sigma_{t-1}^2 + \alpha(|z_{t-1}|-E|z|)+\gamma z_{t-1}",
        "log volatility recursion",
        "garch diagnostics",
        "diagnostic",
    ),
    ...
    rows = pd.DataFrame(
        [
            ...
            {"field": "role", "value": role},
            {
                ...
                "value": "Mathematical diagnostic display. It does not imply active allocation impact unless role says active.",
            },
        ]
    )
