...
def _header(st, bundle) -> None:
    ...
    with cols[2]:
        metric_card(
            st,
            "Signal / Application",
            f"{k.get('signal_date', 'n/a')} -> {k.get('economic_application_date', 'n/a')}",
            "t+1 economic application",
        )
    with cols[3]:
    ...
    with cols[3]:
        metric_card(
            st, "Decision Status", bundle.status.replace("decision_engine_", ""), "dashboard/export validation", badge=bundle.status
        )
...
def _sankey_and_funnel(st, bundle) -> None:
    ...
    with right:
        ...
        if f.empty or "stage" not in f.columns:
            ...
            fig = px.funnel(
                f, y="stage", x="passed_count", color="stage", color_discrete_sequence=DECISION_SEQUENCE, title="Current Decision Funnel"
            )
            _safe_chart(st, fig, 560)
            biggest = f.sort_values("excluded_count", ascending=False).head(1)
            reason = (
                _format_reason_list(bundle.rejected.get("exact_exclusion_reason", pd.Series(dtype=str)))
                if not bundle.rejected.empty
                else "unavailable"
            )
            if not biggest.empty:
                ...
                metric_card(
                    st,
                    "Largest Exclusion Stage",
                    str(row.get("stage", "unavailable")),
                    f"excluded {fmt_num(row.get('excluded_count', np.nan), 0)} · reasons: {reason}",
                )
...
def _selected_explainability(st, bundle) -> None:
    ...
    cols = st.columns(4)
    with cols[0]:
        metric_card(st, "Company", str(row.get("company_name", ticker)), str(row.get("exchange", "exchange unavailable")))
    with cols[1]:
        metric_card(st, "Sector / Industry", str(row.get("sector", "unavailable")), str(row.get("industry", "industry unavailable")))
    with cols[2]:
        metric_card(
            st,
            "Raw Target Rank",
            fmt_num(row.get("raw_target_rank", np.nan), 0),
            f"percentile {fmt_pct(row.get('rank_percentile', np.nan))}",
        )
    with cols[3]:
        metric_card(
            st,
            "Official Weight",
            fmt_pct(row.get("current_official_weight", row.get("paper_position_weight", np.nan))),
            f"target {fmt_pct(row.get('final_target_weight', np.nan))}",
        )
    cols = st.columns(4)
    with cols[0]:
        metric_card(st, "Raw Target Return", fmt_pct(row.get("raw_target_return_exact", np.nan)), "exact source when available")
    with cols[1]:
        metric_card(st, "Distance From Cutoff", fmt_num(row.get("distance_from_portfolio_cutoff", np.nan), 0), "rank distance")
    with cols[2]:
        metric_card(st, "Volatility", fmt_pct(row.get("realized_vol_60d", np.nan)), "60D annualized")
    with cols[3]:
        metric_card(
            st,
            "Risk Contribution",
            fmt_pct(row.get("pct_total_portfolio_risk", np.nan)),
            f"beta {fmt_num(row.get('beta_to_portfolio', row.get('beta_to_spy', np.nan)), 2)}",
        )
    ...
    for col in FEATURE_COLUMNS:
        ...
        percentile = (
            vals.rank(pct=True).iloc[-1] if len(vals) and pd.notna(pd.to_numeric(pd.Series([value]), errors="coerce").iloc[0]) else np.nan
        )
        feature_rows.append(
            {
                "feature": col,
...
def _rejected_and_compare(st, bundle) -> None:
    ...
    if rejected.empty:
        ...
        cols = [
            ...
            "distance_from_portfolio_cutoff",
        ]
    ...
    if not comp.empty and opts:
        ...
        fig = px.bar(
            c.dropna(subset=[metric]),
            x="ticker",
            y=metric,
            color="group",
            barmode="group",
            color_discrete_sequence=DECISION_SEQUENCE,
            title=f"Selected vs Top Rejected — {metric}",
        )
...
def _rule_path_and_pending(st, bundle) -> None:
    ...
    with left:
        ...
        if tree.empty:
            ...
            fig = px.bar(
                ...
                color="status",
                orientation="h",
                color_discrete_map={"PASS": CHART_COLORS.get("positive", "#18C48F"), "FAIL": CHART_COLORS.get("negative", "#FF4D4D")},
                title=f"Rule Path — {ticker}",
            )
    ...
    with right:
        ...
        if pending.empty:
            ...
            metric_card(
                st,
                "Sessions Until Next Rebalance",
                fmt_num(bundle.kpis.get("sessions_until_next_rebalance", np.nan), 0),
                "Signal only — not executed until scheduled rebalance.",
            )
    ...
    with acol:
        ...
        if attr.empty:
            ...
            if not stage_rows.empty:
                fig = px.bar(
                    stage_rows, x="stage", y="count", color="stage", color_discrete_sequence=DECISION_SEQUENCE, title="Exclusions by Stage"
                )
...
def _pipeline_sources(st, bundle) -> None:
    section_header(
        st, "Pipeline Explainability and Sources", "Official/current source audit. No debug namespace mixing in current decision view."
    )
    ...
    if not bundle.missing_attribution.empty:
        alert_box(
            st,
            "Missing attribution data: exact marginal contribution weights are not stored. The terminal shows stored feature values only.",
            "warning",
        )
